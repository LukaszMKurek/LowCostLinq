...
namespace LowCostLinq
{
        ...
        where TFilter1 : struct, IFilter<TIn, TOut>
    {
        ...
        private sealed class EnumeratorArray : IEnumerator<TOut> // todo ienumerable i ToEnumerabele
        {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            internal EnumeratorArray(TIn[] array, in TFilter1 filter1)
            {
                _currentIndex = 0u;
                _array = array;
                _filter1 = filter1;
            ...
            public bool MoveNext()
            {
                ...
                do
                {
                    ...
                    {
                        if (_filter1.Filter(ref _array[_currentIndex++], out _current, ref willBreak))
                        {
                            return true;
                        }
                    }
        ...
        private sealed class EnumeratorListWithoutCheckVersion : IEnumerator<TOut> // todo ienumerable i ToEnumerabele
        {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            internal EnumeratorListWithoutCheckVersion(List<TIn> list, in TFilter1 filter1)
            {
                _currentIndex = 0;
                _list = list;
                _filter1 = filter1;
        ...
        private sealed class EnumeratorList : IEnumerator<TOut> // todo ienumerable i ToEnumerabele
        {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            internal EnumeratorList(List<TIn> list, in TFilter1 filter1)
            {
        ...
        private sealed class EnumeratorEnumerable : IEnumerator<TOut> // todo ienumerable i ToEnumerabele
        {
            ...
            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            internal EnumeratorEnumerable(IEnumerable<TIn> enumerable, in TFilter1 filter1)
            {
        ...
        IEnumerator<TOut> IEnumerable<TOut>.GetEnumerator()
        {
            if (typeof(TCollectionWrapper) == typeof(CollectionWrappers.ArrayW<TIn>))
                return new EnumeratorArray(((CollectionWrappers.ArrayW<TIn>)(object)_collection)._array, _filter1);

            if (typeof(TCollectionWrapper) == typeof(CollectionWrappers.ListW<TIn>))
                return new EnumeratorList(((CollectionWrappers.ListW<TIn>)(object)_collection)._list, _filter1);

            if (typeof(TCollectionWrapper) == typeof(CollectionWrappers.ListWWithoutCheckVersion<TIn>))
                return new EnumeratorListWithoutCheckVersion(((CollectionWrappers.ListWWithoutCheckVersion<TIn>)(object)_collection)._list, _filter1);
            ...
            if (_collection is CollectionWrappers.IEnumerableWrapper<TIn>)
            {
                ...
                if (enumerable is TIn[])
                    return new EnumeratorArray((TIn[])enumerable, _filter1);

                if (enumerable is List<TIn>)
                    if (_collection is CollectionWrappers.IEnumerableWithoutCheckVersionWrapper<TIn>)
                        return new EnumeratorListWithoutCheckVersion((List<TIn>)enumerable, _filter1);
                    else
                        return new EnumeratorList((List<TIn>)enumerable, _filter1);

                return new EnumeratorEnumerable(enumerable, _filter1);
            }
        ...
        public TOut FirstOrDefault()
        {
            ...
            _collection.Iterate<TFilter1, TOut, FirstAccumulator<TOut>>(_filter1, ref acc);
        ...
        public TOut FirstOrDefault(Func<TOut, bool> @where)
        {
            ...
            _collection.Iterate<TFilter1, TOut, FirstWhereAccumulator<TOut>>(_filter1, ref acc);
        ...
        public TOut[] ToArray()
        {
            ...
            _collection.Iterate<TFilter1, TOut, ToArrayAccumulatorOptimizedForDefault<TOut>>(_filter1, ref acc);
